...
- 新增持久化研发 Runner：以 SQLite 租约和检查点支持多 Worker 唯一领取与重启恢复，默认双预算为 12 轮或 60 分钟。
- Git 项目使用独立 worktree/分支，非 Git 项目使用受控副本；模型只能写安全相对路径并运行 Python/Node 白名单验证命令，完成后必须审阅差异并显式应用。
- 新增研发运行、步骤、产物、差异、继续、取消和应用 API；软件 Hub 与 Agent Runs 展示实时阶段、测试日志和应用状态。
- 专家团队卡片增加中文用途和“去使用”深链；论文/笔记上下文要求选择 1–20 个同空间实体。
- 右下角 AI 助手删除本地关键词执行器，改为与 Chat Hub 共享会话、历史和 LLM 流式生成；未配置模型时提供设置入口。
- 新增离线研发工作区 QA，应用版本统一为 `0.5.0`；SwanLab 实验数据流保持不变。
