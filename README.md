...
0.5.0 新增实际 Agent 研发工作区：软件研发团队会在 Git worktree 或受控目录副本中分析、写代码、运行 Python/Node 验证并审查，用户确认差异后才应用到原项目。专家团队卡片可直接跳到适用 Hub；右下角 AI 助手现已与 Chat Hub 共享真实 LLM 会话。
...
| **专家团队** | 多角色 Agent 编排 | 可视化静态 DAG、固定研发流程、角色模板、导入导出、明确“去使用”入口 |
| **研发实验** | 实际软件研发 + 实验追踪 | **隔离代码工作区**、自动实现/测试/审查、差异显式应用、SwanLab 实验对比 |
