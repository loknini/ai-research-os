...
| [DATA-MODEL.md](./DATA-MODEL.md) | 数据模型 | SQLite WAL 与连接管理、space-key 软隔离模型与迁移、29 张业务表、`database.py` 函数分组、更新语义（`rowcount>0`）、QA 验收脚本 |
| [API.md](./API.md) | 接口契约 | 通用约定（X-Space-Key / 响应体 / camelCase）、`/api/*` 路由按 21 个 router 分表、SSE 帧格式、curl 速查 |
